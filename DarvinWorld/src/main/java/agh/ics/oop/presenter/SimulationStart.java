...
import agh.ics.oop.model.TunnelMap;
...
public class SimulationStart {
    ...
    public void onSimulationStartClicked(){
        ...
        MapProperties mapProperties = new MapProperties(Integer.parseInt(width.getText()),Integer.parseInt(height.getText()),Integer.parseInt(startPlant.getText()),Integer.parseInt(plantsPerDay.getText()),Integer.parseInt(energy1Plant.getText()),Integer.parseInt(startAnimal.getText()),true,Integer.parseInt(tunnelCount.getText()));
        AnimalProperties animalProperties = new AnimalProperties(Integer.parseInt(startAnimalEnergy.getText()),Integer.parseInt(breedEnergy.getText()),Integer.parseInt(procreateEnergy.getText()),Integer.parseInt(minMutationNumber.getText()),Integer.parseInt(maxMutationNumber.getText()),Integer.parseInt(genomeLength.getText()),false);
        TunnelMap recmap = new TunnelMap(mapProperties,animalProperties);
        BorderPane viewRoot;
        Stage sims = new Stage();
        FXMLLoader loader = new FXMLLoader();
        loader.setLocation(getClass().getClassLoader().getResource("simulation.fxml"));
        try {viewRoot = loader.load(); } catch (Exception e){
            e.printStackTrace();
            return ;
        }
        ...
        simulations.add(new Simulation(recmap));
